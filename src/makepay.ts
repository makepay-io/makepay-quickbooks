...
import type { AppConfig, MakePayWebhookEvent, QuickBooksInvoice } from './types.js';
...
export async function createPaymentLinkForInvoice(
  ...
  const amount = Number(invoice.Balance ?? invoice.TotalAmt ?? 0);
  const response = await fetch(`${config.makePayBaseUrl}/api/partner/v1/makepay/payment-links`, {
    ...
    body: JSON.stringify({
      ...
      payload: {
        title: `QuickBooks invoice ${invoice.DocNumber ?? invoice.Id}`,
        description: `Payment for QuickBooks invoice ${invoice.DocNumber ?? invoice.Id}`,
        amount: String(amount),
        currency: invoice.CurrencyRef?.value,
        orderId: invoice.Id,
        merchantOrderId: `${realmId}:${invoice.Id}`,
        metadata: {
          platform: 'quickbooks',
          realmId,
